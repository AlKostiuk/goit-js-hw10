...
function countryElementInfo(elements) {
  const resultElementInfo = elements
    ...
    <img class = one_country_flag src= "${element.flags.svg}"/>
        ...
        <p class = country_capital ><strong> Capital:</strong> ${
          element.capital
        } </p>
        <p class = country_population > <strong> Population:</strong>${
          element.population
        } </p>
        <p class = country_language ><strong> Languages:</strong> ${Object.values(
          element.languages
        ).join(',')} </p>`;
    })
    .join('');
